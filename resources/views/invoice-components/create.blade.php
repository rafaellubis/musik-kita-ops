<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                <h2 class="font-semibold text-xl text-mk-text">Tambah Komponen Tagihan</h2>
                <div class="text-xs text-mk-muted mt-0.5">Katalog Item Invoice</div>
            </div>
    ...
    <div class="py-6 px-4 lg:px-8">
        <div class="bg-white shadow-sm sm:rounded-lg p-6 max-w-2xl">
            <form action="{{ route('invoice-components.store') }}" method="POST">
                @csrf
                @include('invoice-components._form')
                <div class="flex justify-end gap-2 mt-6">
                    ...
                    <button type="submit"
                            ...
                            style="background:#D4A853;color:#1A1000">Simpan</button>
                </div>
            </form>
        </div>
    </div>
