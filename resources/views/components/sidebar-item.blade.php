...
<a href="{{ route($route) }}"
   @class([
       'flex items-center gap-2.5 px-3 py-2 rounded-lg transition-all duration-150 select-none',
       // state aktif
       'bg-mk-accentDim text-mk-accent font-semibold border-l-[3px] border-mk-accent pl-[9px]' => $active,
       // state normal
       'text-mk-muted hover:bg-white/5 hover:text-mk-text border-l-[3px] border-transparent'   => !$active,
   ])>
