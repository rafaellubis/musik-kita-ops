<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                <h2 class="font-semibold text-xl text-mk-text">Master Data — Guru</h2>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div class="bg-mk-card rounded-xl border border-mk-borderLight shadow-sm p-5 fade-in-up
                 ...
                 style="animation-delay:{{ $idx * 60 }}ms;
                        --hover-border:rgba(93,184,144,0.4)"
                ...
                <div class="flex justify-between items-start mb-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <div class="text-sm font-bold text-mk-text">{{ $t->name }}</div>
                            <div class="text-xs font-mono text-mk-dim">{{ $t->code }}</div>
                        </div>
                    </div>
                    {{-- Badge status --}}
                    @if($t->is_active)
                ...
                <div class="flex flex-wrap gap-1.5 mb-4">
                    ...
                    <span class="px-2 py-0.5 rounded-full text-xs font-semibold"
                          style="{{ $ins->pivot->is_primary
                              ? 'background:rgba(93,184,144,0.18);color:#5DB890'
                              : 'background:rgba(255,255,255,0.06);color:#8B92A8' }}">
                        {{ $ins->pivot->is_primary ? '★ ' : '' }}{{ $ins->name }}
                    </span>
                    @empty
                    <span class="text-xs text-mk-dim">Belum ada instrumen</span>
                    @endforelse
                </div>
                ...
                <div class="grid grid-cols-2 gap-2 mb-4">
                    <div class="rounded-lg p-2.5"
                        ...
                        <div class="text-[10px] uppercase tracking-wide text-mk-dim mb-1">Murid Aktif</div>
                        <div class="text-lg font-bold text-mk-text">{{ $t->active_students ?? 0 }}</div>
                    </div>
                    <div class="rounded-lg p-2.5"
                         style="background:rgba(255,255,255,0.04)">
                        <div class="text-[10px] uppercase tracking-wide text-mk-dim mb-1">Instrumen</div>
                        <div class="text-lg font-bold text-mk-text">{{ $t->instruments->count() }}</div>
                    </div>
                </div>

                {{-- Email (jika ada) --}}
                @if($t->email)
                <div class="text-xs text-mk-dim mb-4 truncate">{{ $t->email }}</div>
                @endif

                {{-- Aksi --}}
                @role('Owner|Admin')
                <div class="flex gap-2 pt-3 border-t border-mk-borderLight">
