<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                <h2 class="font-semibold text-xl text-mk-text">Edit Pengeluaran Petty Cash</h2>
                <div class="text-xs text-mk-muted mt-0.5">
                    {{ $expense->expense_number }} · Saldo tersedia: Rp {{ number_format($balance, 0, ',', '.') }}
                </div>
            </div>
            <a href="{{ route('petty-cash.expenses.show', $expense) }}"
    ...
    <div class="py-6 px-4 lg:px-8">
        <div class="bg-mk-card shadow-sm sm:rounded-lg p-6 max-w-2xl">
            <form method="POST" action="{{ route('petty-cash.expenses.update', $expense) }}" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
