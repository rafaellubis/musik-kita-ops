<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('petty-cash.index', ['year' => $expense->expense_date->year, 'month' => $expense->expense_date->month]) }}"
                ...
                <div class="text-right">
                    ...
                    <div class="flex gap-2 mt-2 justify-end">
                        @hasanyrole('Owner|Admin')
                            <a href="{{ route('petty-cash.expenses.edit', $expense) }}"
                               class="px-3 py-1 bg-indigo-600 text-white rounded text-xs hover:bg-indigo-700">
                                Edit
                            </a>
                        @endhasanyrole
                        @if($isOwner)
                            <form method="POST" action="{{ route('petty-cash.expenses.destroy', $expense) }}"
                                  onsubmit="return confirm('Hapus pengeluaran petty cash ini?')">
                                @csrf @method('DELETE')
                                <button type="submit"
        ...
        @if($expense->receipt_image)
            <div class="bg-mk-card shadow-sm sm:rounded-lg p-4">
                ...
                <img src="{{ asset('storage/' . $expense->receipt_image) }}"
                     alt="Bukti pengeluaran petty cash"
